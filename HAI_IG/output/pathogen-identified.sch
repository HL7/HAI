...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Observation">
      ...
      <sch:assert test="not(exists(f:*[starts-with(local-name(.), 'value')])) or count(for $coding in f:code/f:coding return self::*/f:component/f:code/f:coding[f:code/@value=$coding/f:code/@value and f:system/@value=$coding/f:system/@value])=0">If code is the same as a component code then the value element associated with the code SHALL NOT be present (inherited)</sch:assert>
